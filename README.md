...
- **跨平台构建验证**：GitHub Actions 对 macOS arm64/x64、Windows x64 和 Linux x64 执行原生构建、测试、打包与安装包冒烟；发布签名和公证仍属于计划路线。

## 实验性能力

- Mermaid 错误图表会安全回退为源码块，专用诊断占位仍在完善。
- Excalidraw 支持标准场景编辑与保存；与 Markdown 关联的 SVG/PNG 自动重导出尚未交付。
- PDF 和 DOCX 可在工作区预览；富文本粘贴、可验证的高保真转换和 PDF 导出不属于当前承诺。

## 计划路线

- 自动保存、崩溃草稿与统一的原子文档写入。
- 文件关联、`mmd <file-or-directory>` CLI、单实例路由、签名、公证和更新回滚。
- 工作区全文搜索、资源管线、HTML/PNG/PDF 导出及可复现性能门禁。
